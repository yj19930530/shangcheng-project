<template>
	...
	<view :style="[{ marginTop: navHeight + 'px' }]" v-if="pageLoading">
		<!-- 头部导航 -->
		...
		<div style="margin-top: -100rpx; padding-bottom: 20rpx">
			<swiper class="swiper swiper-container" :indicator-dots="indicatorDots" :autoplay="autoplay" :interval="interval"
			 :duration="duration">
				<swiper-item v-for="(item, index) in homePageData[2].emsModalGoodVo" :key="index">
					<image mode="aspectFill" @tap="navToDetail(item)" class="swiper-item-img" :src="httpImg + item.indexDisplayPic" />
				</swiper-item>
			</swiper>
			<!-- 商品内容 -->
			<div class="best-container">
				...
				<div class="shops-list-content fl-btw">
					<image @tap="navToDetail(item)" v-for="item in homePageData[1].emsModalGoodVo" :key="item.id" class="shops-item-img"
				...
				<div class="new-shop-list fl-co">
					<image mode="aspectFill" @tap="navToDetail(item)" v-for="item in homePageData[0].emsModalGoodVo" :key="item.id"
					 class="new-shop-img" :src="httpImg + item.indexDisplayPic" />
				</div>
			</div>
		</div>
		<div class="fl-co" style="margin-top: -20rpx; padding-bottom: 20rpx">
			<image class="goods-shuai-xuan" @tap="toGoodsPage(2)" src="../../static/home/qudou.png"></image>
			<image @tap="toGoodsPage(3)" class="goods-shuai-xuan" src="../../static/home/meibai.png"></image>
			<image @tap="toGoodsPage(1)" class="goods-shuai-xuan" src="../../static/home/bushui.png"></image>
		</div>
	</view>
</template>
<script>
	...
	export default {
		...
		methods: {
			...
			async getMsgNumber() {
				const {
					data
				} = await this.$api.getMessageCount();
				this.msgNumber = data;
			},
			toHufuPath() {
